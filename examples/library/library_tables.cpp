...
const db_fields_collection book_fields = {
  ...
  db_variable(TABLE_FIELD_PAIR(BOOK_PUB_YEAR), db_type::type_int,
      { .can_be_null = false }),
  db_variable(TABLE_FIELD_PAIR(BOOK_LANG), db_type::type_int,
      { .can_be_null = false }),
};
...
static const db_table_create_setup book_create_setup(
    table_book, book_fields, book_uniques, nullptr);
...
const db_fields_collection translation_fields = {
  ...
  db_variable(TABLE_FIELD_PAIR(TRANS_LANG), db_type::type_int,
      { .can_be_null = false }),
  db_variable(TABLE_FIELD_PAIR(TRANS_TRANS_TITLE), db_type::type_text,
      { .can_be_null = false }),
  db_variable(TABLE_FIELD_PAIR(TRANS_TRANSLATORS), db_type::type_text,
      { .can_be_null = false }),
};
...
static const db_table_create_setup author_create_setup(
    table_author, author_fields, aut_uniques, nullptr);
}  // namespace table_fields_setup
...
void IDBTables::SetSelectData<book>(db_query_select_result *src,
  ...
  for (auto &row: src->values_vec) {
    ...
    for (auto &col: row) {
      ...
      } else if (src->isFieldName(TABLE_FIELD_NAME(BOOK_TITLE), src->fields[col.first])) {
        b.title = col.second.c_str();
      } else if (src->isFieldName(TABLE_FIELD_NAME(BOOK_PUB_YEAR), src->fields[col.first])) {
        b.first_pub_year = std::atoi(col.second.c_str());
      } else if (src->isFieldName(TABLE_FIELD_NAME(BOOK_LANG), src->fields[col.first])) {
...
void IDBTables::SetSelectData<translation>(db_query_select_result *src,
  ...
  for (auto &row: src->values_vec) {
    translation tr;
    for (auto &col: row) {
      if (src->isFieldName(TABLE_FIELD_NAME(TRANS_ID), src->fields[col.first])) {
        tr.id = std::atoi(col.second.c_str());
      } else if (src->isFieldName(TABLE_FIELD_NAME(TRANS_BOOK_ID), src->fields[col.first])) {
        tr.book_p.first = std::atoi(col.second.c_str());
      } else if (src->isFieldName(TABLE_FIELD_NAME(TRANS_LANG), src->fields[col.first])) {
        tr.lang = (language_t)std::atoi(col.second.c_str());
      } else if (src->isFieldName(TABLE_FIELD_NAME(TRANS_TRANS_TITLE), src->fields[col.first])) {
        tr.translated_name = col.second.c_str();
      } else if (src->isFieldName(TABLE_FIELD_NAME(TRANS_TRANSLATORS), src->fields[col.first])) {
...
void IDBTables::SetSelectData<author>(db_query_select_result *src,
  ...
  for (auto &row: src->values_vec) {
    author a;
    for (auto &col: row) {
      if (src->isFieldName(TABLE_FIELD_NAME(AUTHOR_ID), src->fields[col.first])) {
        a.id = std::atoi(col.second.c_str());
      } else if (src->isFieldName(TABLE_FIELD_NAME(AUTHOR_NAME), src->fields[col.first])) {
        a.name = col.second.c_str();
      } else if (src->isFieldName(TABLE_FIELD_NAME(AUTHOR_BORN_YEAR), src->fields[col.first])) {
